...
BSTnode* ExpressionTree(BSTnode* node, string e) {
    // step 1 check for lowest precedence operator and make it node node
    char op = '^';
    char Nodechar= '-';
    char ch;
    int index = 0;
    ...
    for (int i = 0; e[i]; i++) {  //slice the expression E[I] = I<SIZE
        ch = e[i];
        ...
        if (!isalnum(ch) && precedence(op) >= precedence(ch)) {   // if operator
            // uses right associative strategy -- 2 same prece right sai solve krega
            op = ch;                                        // if same precedence operator right most side one is taken
            index = i; //note index where we work
            cout << op << endl;
        }
    ...
    // if expression is node single char
    if (e.length() == 1)
    {
        cout << "in final if  string is node single char\n";
